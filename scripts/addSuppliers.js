...
async function main() {
  ...
  const tx1 = await registry.connect(acc4).registerStakeholder(role1, metadata1);
  await tx1.wait();
  console.log(`Stakeholder 1 registered as role ${role1}`);

  // Register Stakeholder 2 using stakeholder2 account
  const tx2 = await registry.connect(acc5).registerStakeholder(role2, metadata2);
  await tx2.wait();
  console.log(`Stakeholder 2 registered as role ${role2}`);

  // Register Stakeholder 3 using stakeholder3 account
  const tx3 = await registry.connect(acc6).registerStakeholder(role3, metadata3);
  await tx3.wait();
